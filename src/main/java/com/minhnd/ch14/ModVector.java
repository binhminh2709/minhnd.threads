...
public class ModVector<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
   ...
   * index <tt>k</tt> in this vector is copied into component <tt>k</tt> of
   * <tt>anArray</tt>. The array must be big enough to hold all the objects in
   * this vector, else an <tt>IndexOutOfBoundsException</tt> is thrown.
   *
   * @param anArray
   ...
   * the field <tt>elementData</tt>, with a smaller one. An application can use
   ...
   * data array will be the old size plus <tt>capacityIncrement</tt>, unless the
   * value of <tt>capacityIncrement</tt> is less than or equal to zero, in which
   * case the new capacity will be twice the old capacity; but if this new size
   * is still smaller than <tt>minCapacity</tt>, then the new capacity will be
   * <tt>minCapacity</tt>.
   *
   * @param minCapacity
   ...
   *         in the field <tt>elementData</tt> of this vector).
   */
  public int capacity() {
   ...
   *         as a component in this vector, as determined by the <tt>equals</tt>
   ...
   *         vector at position <code>index</code> or later in the vector, that
   *         is, the smallest value <tt>k</tt> such that
   *         <tt>elem.equals(elementData[k]) && (k &gt;= index)</tt> is
   *         <tt>true</tt>; returns <code>-1</code> if the object is not found.
   *         (Returns <code>-1</code> if <tt>index</tt> &gt;= the current size
   *         of this <tt>ModVector</tt>.)
   ...
   * @exception IndexOutOfBoundsException
   *              if <tt>index</tt> is greater than or equal to the current size
   *              of this vector.
   */
   ...
   * <p>
   *
   * This method is identical in functionality to the get method (which is part
   ...
   *              if the <tt>index</tt> is negative or not less than the current
   *              size of this <tt>ModVector</tt> object. given.
   * @see #get(int)
   * @see List
   */
  ...
  /**
   * Returns the first component (the item at index <tt>0</tt>) of this vector.
   *
   ...
   * <p>
   *
   * The index must be a value greater than or equal to <code>0</code> and less
   * than the current size of the vector.
   * <p>
   *
   * This method is identical in functionality to the set method (which is part
   * of the List interface). Note that the set method reverses the order of the
   * parameters, to more closely match array usage. Note also that the set
   ...
   * with an index greater or equal to the specified <code>index</code> is
   * shifted downward to have an index one smaller than the value it had
   * previously. The size of this vector is decreased by <tt>1</tt>.
   * <p>
   *
   * The index must be a value greater than or equal to <code>0</code> and less
   * than the current size of the vector.
   ...
   * equal to the specified <code>index</code> is shifted upward to have an
   ...
   * the current size of the vector, the new element is appended to the
   * ModVector.)
   * <p>
   *
   ...
   * <p>
   *
   * This method is identical in functionality to the add(Object) method (which
   ...
   * data array of this <tt>ModVector</tt> object.
   *
   ...
   * of the specified array and the size of this ModVector.
   * <p>
   *
   * If the ModVector fits in the specified array with room to spare (i.e., the
   * array has more elements than the ModVector), the element in the array
   * immediately following the end of the ModVector is set to null. This is
   * useful in determining the length of the ModVector <em>only</em> if the
   * caller knows that the ModVector does not contain any null elements.
   *
   * @param a
   *          the array into which the elements of the ModVector are to be
   *          stored, if it is big enough; otherwise, a new array of the same
   *          runtime type is allocated for this purpose.
   * @return an array containing the elements of the ModVector.
   * @exception ArrayStoreException
   *              the runtime type of a is not a supertype of the runtime type
   *              of every element in this ModVector.
   * @throws NullPointerException
   *           if the given array is null.
   * @since 1.2
   */
  ...
  /**
   * Removes the first occurrence of the specified element in this ModVector If
   * the ModVector does not contain the element, it is unchanged. More formally,
  ...
  /**
   * Removes the element at the specified position in this ModVector. shifts any
   * subsequent elements to the left (subtracts one from their indices). Returns
   * the element that was removed from the ModVector.
   *
   * @exception ArrayIndexOutOfBoundsException
   *              index out of range (index &lt; 0 || index &gt;= size()).
   * @param index
  ...
  /**
   * Removes all of the elements from this ModVector. The ModVector will be
   ...
   *
   * @param c
   *          a collection whose elements will be tested for containment in this
   *          ModVector
   ...
   * Collection is this ModVector, and this ModVector is nonempty.)
   *
   * @param c
   ...
   * specified Collection. In other words, removes from this ModVector all of
   * its elements that are not contained in the specified Collection.
   *
   * @param c
   *          a collection of elements to be retained in this ModVector (all
   ...
   * ModVector at the specified position. Shifts the element currently at that
   * position (if any) and any subsequent elements to the right (increases their
   * indices). The new elements will appear in the ModVector in the order that
   ...
   *
   * @param o
   *          the Object to be compared for equality with this ModVector.
  ...
  /**
   * Returns a string representation of this ModVector, containing the String
   ...
   * used as a range operation by operating on a subList view instead of a whole
   ...
   * to the left (reduces their index). This call shortens the ArrayList by
   * (toIndex - fromIndex) elements. (If toIndex==fromIndex, this operation has
   * no effect.)
   *
   * @param fromIndex
  ...
  /**
   * Save the state of the <tt>ModVector</tt> instance to a stream (that is,
   * serialize it). This method is present merely for synchronization. It just
   * calls the default readObject method.
   */
